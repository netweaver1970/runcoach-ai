      ...
      <Modal visible={customising} animationType="slide" transparent onRequestClose={() => setCustomising(false)}>
        <View style={s.sheetBackdrop}>
          <View style={s.sheet}>
            ...
            {/* Scrollable so all cards are reachable, but scroll is FROZEN while dragging (onDragActive) so the
                grip's pan reorders instead of the ScrollView stealing it. The grip captures the touch on press,
                so grabbing ≡ drags; grabbing elsewhere scrolls. flexShrink lets it scroll inside the capped sheet. */}
            <ScrollView style={{ flexShrink: 1 }} scrollEnabled={!dragActive} showsVerticalScrollIndicator keyboardShouldPersistTaps="handled">
              <ReorderList items={layout} titleOf={(id) => STAT_CARD_TITLES[id as StatCardId]} onCommit={commitLayout} onDragActive={setDragActive} />
            </ScrollView>
          </View>
        </View>
      </Modal>
