/**
 * Expo config plugin — give the watchOS app a COMPLETE app-icon set.
 *
 * @bacons/apple-targets generates only a single 1024×1024 image for the watch
 * target's AppIcon. watchOS (unlike iOS) does NOT synthesize the home-screen icon
 * sizes from a single image, so the app shipped with no usable icon → a grey
 * placeholder in the Watch app and "could not be installed at this time".
 *
 * This plugin runs AFTER apple-targets (it must be listed after it in app.json's
 * plugins array) and rewrites targets/watch/Assets.xcassets/AppIcon.appiconset with
 * the full watchOS icon set, rasterised from assets/icon.png with `sips`. Because
 * actool reads that folder at xcodebuild time, the final on-disk state wins.
 */

const { withDangerousMod } = require('@expo/config-plugins');
const { execFileSync } = require('child_process');
const fs = require('fs');
const path = require('path');

// size(px) → watchOS icon role/subtype (legacy explicit set; watchOS needs these).
const ICONS = [
  { px: 48,   idiom: 'watch', role: 'notificationCenter', subtype: '38mm', size: '24x24',     scale: '2x' },
  { px: 55,   idiom: 'watch', role: 'notificationCenter', subtype: '42mm', size: '27.5x27.5', scale: '2x' },
  { px: 58,   idiom: 'watch', role: 'companionSettings',                   size: '29x29',     scale: '2x' },
  { px: 87,   idiom: 'watch', role: 'companionSettings',                   size: '29x29',     scale: '3x' },
  { px: 80,   idiom: 'watch', role: 'appLauncher',        subtype: '38mm', size: '40x40',     scale: '2x' },
  { px: 88,   idiom: 'watch', role: 'appLauncher',        subtype: '40mm', size: '44x44',     scale: '2x' },
  { px: 92,   idiom: 'watch', role: 'appLauncher',        subtype: '41mm', size: '46x46',     scale: '2x' },
  { px: 100,  idiom: 'watch', role: 'appLauncher',        subtype: '44mm', size: '50x50',     scale: '2x' },
  { px: 102,  idiom: 'watch', role: 'appLauncher',        subtype: '45mm', size: '51x51',     scale: '2x' },
  { px: 108,  idiom: 'watch', role: 'appLauncher',        subtype: '49mm', size: '54x54',     scale: '2x' },
  { px: 172,  idiom: 'watch', role: 'quickLook',          subtype: '38mm', size: '86x86',     scale: '2x' },
  { px: 196,  idiom: 'watch', role: 'quickLook',          subtype: '42mm', size: '98x98',     scale: '2x' },
  { px: 216,  idiom: 'watch', role: 'quickLook',          subtype: '44mm', size: '108x108',   scale: '2x' },
  { px: 234,  idiom: 'watch', role: 'quickLook',          subtype: '45mm', size: '117x117',   scale: '2x' },
  { px: 258,  idiom: 'watch', role: 'quickLook',          subtype: '49mm', size: '129x129',   scale: '2x' },
  { px: 1024, idiom: 'watch-marketing',                                    size: '1024x1024', scale: '1x' },
];

module.exports = function withWatchAppIcon(config) {
  return withDangerousMod(config, [
    'ios',
    async (cfg) => {
      const root = cfg.modRequest.projectRoot;
      const src = path.join(root, 'assets', 'icon.png');
      const set = path.join(root, 'targets', 'watch', 'Assets.xcassets', 'AppIcon.appiconset');

      if (!fs.existsSync(src)) {
        console.warn('[withWatchAppIcon] assets/icon.png not found — skipping');
        return cfg;
      }
      if (!fs.existsSync(set)) {
        // apple-targets hasn't generated the watch target yet (not listed after it?).
        console.warn(`[withWatchAppIcon] ${set} not found — is this plugin after @bacons/apple-targets? skipping`);
        return cfg;
      }

      try {
        // Wipe apple-targets' single-size icon, then rasterise the full set.
        for (const f of fs.readdirSync(set)) fs.rmSync(path.join(set, f));
        const images = ICONS.map(({ px, ...meta }) => {
          const filename = `icon_${px}.png`;
          execFileSync('sips', ['-z', String(px), String(px), src, '--out', path.join(set, filename)], { stdio: 'ignore' });
          return { ...meta, filename };
        });
        fs.writeFileSync(
          path.join(set, 'Contents.json'),
          JSON.stringify({ images, info: { version: 1, author: 'runcoach-watch-icon' } }, null, 2)
        );
        console.log(`[withWatchAppIcon] wrote ${images.length} watchOS app-icon sizes ✓`);
      } catch (e) {
        console.warn('[withWatchAppIcon] icon generation failed (need macOS `sips`):', e.message);
      }
      return cfg;
    },
  ]);
};
