...
5. CRITICAL — the prescription ABOVE is the ONLY plan to judge this run against. Today's CURRENT plan (e.g. from get_plan_context) may now read "rest / recover / session done" — that is because this run is ALREADY COMPLETED and the coach flipped to recovery AFTERWARDS. It does NOT mean a rest day was prescribed. NEVER say the athlete "ran on a rest day", "was prescribed rest", or "should have rested" — this run WAS the prescribed session. Ignore the post-run current-plan state entirely.`;
}
...
export async function analyzeRun(
  ...
  const raw = await agentComplete({ system: SYSTEM_PROMPT, messages: [{ role: 'user', content: userMsg }], maxTokens: 4000, snap });
